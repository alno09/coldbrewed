...
type BrewItemDraftWithId = BrewItemDraft & {
  localId: string;
};
...
const createDraftId = () => `draft-${nextDraftId++}`;

const createEmptySubBatch = (
...
export default function AddBatch() {
  ...
  const [batchForm, setBatchForm] = useState<Required<NewBatchInput>>({
    ...
    ratio: '1:10',
    water: '',
    methodGroup: 'Full Fridge',
  });
  const [subBatches, setSubBatches] = useState<BrewItemDraftWithId[]>([
    createEmptySubBatch(),
  ]);
  ...
  const updateSubBatchField = <K extends keyof BrewItemDraftWithId>(
    localId: string,
    key: K,
    value: BrewItemDraftWithId[K]
  ) => {
  ...
  const handleSave = () => {
    ...
    const processedItems: NewBrewItemInput[] = subBatches.map(
      ({ localId: _localId, ...item }) => ({
        ...item,
        totalScore:
          item.fragrance +
          item.acidity +
    ...
    >
      ...
      >
        ...
        <View className="bg-[#1E1E1E] p-4 rounded-2xl mb-6">
          ...
          />
          <Text className="text-gray-500 mb-2 uppercase tracking-widest text-[10px] font-bold">
            Cold Brew Method
          </Text>
          <TextInput
            className="text-gray-300 border-b border-[#333] pb-2"
            placeholder="Cold Brew Method (e.g. Full Fridge, Half Bloom)"
            placeholderTextColor="#666"
        ...
        {subBatches.map((sb) => (
          ...
          >
            <TextInput
              ...
              value={sb.label}
              onChangeText={(value) =>
                updateSubBatchField(sb.localId, 'label', value)
              }
            />
            <TextInput
              ...
              onChangeText={(value) =>
                updateSubBatchField(sb.localId, 'duration', Number(value) || 0)
              }
            />
            <TextInput
              className="bg-[#121212] text-white p-3 rounded-lg mb-3"
              ...
              onChangeText={(value) =>
                updateSubBatchField(sb.localId, 'harvestTime', value)
              }
            />
              ...
              {scoreFields.map((attr) => (
                <View key={attr} className="w-[48%] mb-2">
                  ...
                  <TextInput
                    ...
                    onChangeText={(value) =>
                      updateSubBatchField(sb.localId, attr, Number(value) || 0)
                    }
                  />
                </View>
              ))}
            ...
            <TextInput
              ...
              value={sb.notes}
              onChangeText={(value) =>
                updateSubBatchField(sb.localId, 'notes', value)
              }
            />
          </View>
        ))}
        ...
        <TouchableOpacity
          onPress={() =>
            setSubBatches((current) => [
              ...current,
              createEmptySubBatch(`Sub-Batch ${current.length + 1}`),
            ])
          }
